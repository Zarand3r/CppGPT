// encoder_forward / encoder_backward: token + learned-position embedding.
...
int main() {
    ...
    // ---- forward ----
    {
        ...
        encoder_forward(out.data(), tokens.data(), wte.data(), wpe.data(), B, T, C, V, Device::CPU);
    ...
    // ---- backward: scatter-add, with a repeated token to test accumulation ----
    {
        ...
        encoder_backward(dwte.data(), dwpe.data(), tokens.data(), dout.data(), B, T, C, V,
                         Device::CPU);
