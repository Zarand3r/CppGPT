...
int main() {
    // ---- forward ----
    {
        ...
        float out[3] = {0, 0, 0};
        softmax_forward(out, in, 3, Device::CPU);
    ...
    // ---- backward: finite-difference gradient check ----
    {
        ...
        softmax_forward(out.data(), inp.data(), N, Device::CPU);
        softmax_backward(dinp.data(), dout.data(), out.data(), N, Device::CPU);
