...
enum class ArtifactKind : std::uint32_t {
    ...
    ProbeDirections = 2,   // per (layer, property): decodability and its causal test
    InductionScores = 3,   // per head: attention on repeated blocks, against a control
};
...
// ---------------------------------------------------------------------------
// ProbeDirections payload
// ---------------------------------------------------------------------------
//
// One record per (layer, property). Both halves travel together on purpose: a
// decodability number without its causal test is the claim this repo has twice
// had to retract, so the format does not let them be separated.
struct ProbeHeader {
    std::uint32_t n_layer;
    std::uint32_t n_props;
    float scale;          // steering scale, in units of residual norm
    std::uint32_t n_null; // draws in the random null
};
static_assert(sizeof(ProbeHeader) == 16, "ProbeHeader must be 16 bytes on disk");

struct ProbeRecord {
    char name[16];        // property, null-padded
    std::uint32_t layer;
    float accuracy;       // held-out
    float base_rate;      // majority class -- accuracy is unreadable without it
    float shuffled;       // the leakage control
    float steer_kl;
    float null_mean;
    float beats_null;     // fraction of the null the direction exceeds
};
static_assert(sizeof(ProbeRecord) == 44, "ProbeRecord must be 44 bytes on disk");

// ---------------------------------------------------------------------------
// InductionScores payload
// ---------------------------------------------------------------------------
//
// `control` is not optional decoration: the raw score is unreadable without it,
// because some attention lands on those positions by geometry alone. A head is
// only doing induction if `repeated` exceeds BOTH `control` and `uniform`.
struct InductionHeader {
    std::uint32_t n_layer;
    std::uint32_t n_head;
    std::uint32_t trials;
    std::uint32_t half;
    float uniform;        // what uniform attention scores on the same positions
    std::uint32_t reserved;
};
static_assert(sizeof(InductionHeader) == 24, "InductionHeader must be 24 bytes on disk");

struct InductionRecord {
    float repeated;
    float control;
};
