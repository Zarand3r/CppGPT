...
// v1 is CPU-only. An earlier design threaded a `Device` tag through every op
// signature as a "seam" for a future CUDA backend; it was removed because no
// caller ever passed it, the enum had one value, and each entry point merely
// asserted that value before calling the one implementation -- the header
// claimed a dispatch that did not exist. When a second backend is real, the seam
// belongs at the arena (which owns the memory), not on 17 leaf-op signatures.
#pragma once
